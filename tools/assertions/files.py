...
from testdata.files.file_schema import CreateFileRequestSchema, CreateFileResponseSchema, FileSchema, \
...
def assert_get_file_with_incorrect_file_id_response(actual: ValidationErrorResponseSchema):
    """
    Проверяет, что ответ на запрос получения файла с некорректным ID соответствует ожидаемой валидационной ошибке.
    ...
    expected = ValidationErrorResponseSchema(
        details=[
            ValidationErrorSchema(
                type="uuid_parsing",  # Тип ошибки, связанной с неверным форматом UUID.
                input="incorrect-file-id",  # Некорректное значение, переданное в качестве ID.
                context={"error": "invalid character: expected an optional prefix of `urn:uuid:` followed by [0-9a-fA-F-], found `i` at 1"},  # Детали ошибки парсинга.
                ...
                location=["path", "file_id"]  # Ошибка возникает в параметрах пути (URL), поле "file_id".
            )
        ]
    )
    assert_validation_error_response(actual, expected)